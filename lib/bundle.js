...
class Pokemon {
  ...
  draw(ctx) {
    if (!this.parsed) {
      ...
      img.src = `https://sprites.pokecheck.org/bw/i/49${num}.gif`;
      img.id = `${this.color}`;
      img.setAttribute("rel:animated_src", `https://sprites.pokecheck.org/bw/i/49${num}.gif`);
      ...
      const play = (res) => {
        ...
        for (let i = 1; i < char.get_length(); i++) {
          let offset = { x: 0, y: i * char.get_canvas().height };
          char.set_frame_offset(i, offset);
        }
        char.set_sizes(char.get_canvas().width, char.get_length() * char.get_canvas().height);
        char.get_canvas().id = `${this.color}`; 
        this.spritesheetCanvas = char.get_canvas();
        this.num_frames = char.get_length();
      }
      char.load(play);
    }
    ...
    if (this.spritesheetCanvas) {
      ...
      // let sourceImageData = this.spritesheetCanvas.toDataURL("image/png");
      ...
      // let image = new Image();
      // image.src = this.spritesheetCanvas.toDataURL("image/png");
