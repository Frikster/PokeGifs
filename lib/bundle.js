...
/* harmony import */ var _player_pokemon__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./player_pokemon */ "./lib/player_pokemon.js");
/* harmony import */ var _ui__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./ui */ "./lib/ui.js");
/* harmony import */ var _spawn_sidebar__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./spawn_sidebar */ "./lib/spawn_sidebar.js");
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _pokemon__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./pokemon */ "./lib/pokemon.js");
...
"use strict";
__webpack_require__.r(__webpack_exports__);
const Stream = __webpack_require__(/*! ../vendor/stream */ "./vendor/stream.js");
const Util = __webpack_require__(/*! ./util */ "./lib/util.js");
...
class Pokemon {
  ...
  handleCollisionWith(otherObject) {
    ...
    const aheadWillBounce = (!pOtherV[0] && pOtherV[1] && !pV[0] && pV[1]) || (pOtherV[0] && !pOtherV[1] && pV[0] && !pV[1]) || allNeg || allPos;
    // const willTeleportThru = (pOtherV[0] && pOtherV[1] && !pV[0] && !pV[1]) || (!pOtherV[0] && !pOtherV[1] && pV[0] && pV[1]); true if others false
    const willBounce = pOtherV.concat(pV).filter(p => p).length === 3 || pOtherV.concat(pV).filter(p => p).length === 1;
    ...
    if (this.isMoving() && otherObject.isMoving()) {
      ...
      if (aheadWillBounce && !this.goingRound) {
        ...
        // debugger
        // if (Util.dist(this.pos, this.destination) < Util.dist(otherObject.pos, otherObject.destination)) {
        ...
        if (this.pos[1] + this.radius <= otherObject.pos[1] + otherObject.radius + 1) {
          const leftSide = [
          ...
          console.log("time to go round:" + Util.dist(previousDestination, this.destination) / Util.speed(this.vel));
          this.goingRound = true;
          setTimeout(function() {
              this.setMotionAndDestination(previousDestination);
              this.goingRound = false;
              console.log("finished going round");
            }.bind(this), 1000);
        }
        // this.move();
        // this.move();
      }
    } else if (this.isMoving() && Util.dist(this.pos, this.destination) <= otherObject.radius + this.radius) {
      ...
      // Go round if pokemon's feet is "inside" the colliding object
      // const previousDestination = this.destination.slice();
      // if(this.pos[1] <= otherObject.pos[1] + 1) {
      //   this.setMotionAndDestination([
      //     otherObject.pos[0] - (otherObject.radius + this.radius),
      //     otherObject.pos[0]
      //   ]);
      //   setTimeout(function () { this.setMotionAndDestination(previousDestination); }, Util.dist(previousDestination, this.destination) / Util.speed(this.vel)).bind(this);
      // }
    }
  }
...
const Util = {
  ...
  // A = right-angle point
  // B = other point
  findThirdPointInRightTriangle(A, B, r, left = 1) {// if left = 1 the D is left of the line AB. Run with -1 to get other side
